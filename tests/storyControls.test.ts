...
test("story choices are absent until the reader opens What next", () => {
...
test("compact video errors preserve the full details and retry, without labeling saved footage Live", () => {
  ...
  assert.match(html, /Video unavailable · saved background/);
...
test("compact subtitles keep accessible full-text and manual navigation", () => {
  const html = renderToStaticMarkup(
  ...
  assert.match(html, /aria-label="Next subtitle"/);
  assert.match(html, /aria-label="Previous subtitle"/);
