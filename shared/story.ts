...
export const PageSchema = z.object({
  ...
  visualUpdate: z.enum(["continue", "redraw"]).optional(),
...
export const GeneratedPageSchema = PageSchema.extend({
  ...
  visualUpdate: z.enum(["continue", "redraw"]),
...
Return title, narrative, question, questionStyle, choices (either two short answers or an empty array for an open-ended question), choiceVisuals, theme, visualPrompt, visualChange, visualUpdate, responseKind, and acknowledgment.
...
Resolve short replies such as "I think they are pink" against the PREVIOUS question. If that question asked about scales, the subject is the established creature's scales, not a new pink object. Apply that appearance permanently to the complete scene and subsequent prepared choices; replace any conflicting old color. The visualChange must foreground the child's contribution: "The dragon's scales turn vivid pink across its whole body, clearly visible in the sunlight." Do not substitute an incidental action such as dipping a paw, walking away, or meeting someone new. Stay on this visual beat so the child can see their idea. Show the affected character and feature clearly in the center of the frame, with its whole body visible.
Set visualUpdate to "redraw" when the child supplies or changes a visible appearance (color, scales, clothing, wings, shape, size), even if phrased as an observation. Also redraw for a major transformation such as becoming airborne from a seated pose. This restarts the current picture on the same connection using the complete new description. Use "continue" for ordinary movements and dialogue, questions, simplification and calming. The visualPrompt for redraw must describe the RESULT as already visible: "a dragon with bright pink scales", never "a green dragon about to turn pink". Do not mention the superseded appearance in that prompt.
